...
contract Auction {
    ...
    modifier canDoActionStartSession {
        require(state == State.CREATED);
        _;
    }
    modifier canDoActionRegister {
        require(state == State.CREATED);
        _;
    }
    
    modifier canDoActionBid {
        require(state == State.STARTED);
        _;
    }

    modifier canDoActionAnnoucement {
        require(state == State.STARTED);
        _;
    }

    modifier canDoActionGetDeposit {
        require(state == State.CLOSING);
        _;
    }
    ...
    // Register new Bidder
    function register(address _account, uint8 _token) canDoActionRegister public {
    ...
    // Start the session.
    function startSession() public canDoActionStartSession {
        state = State.STARTED;
    }
    
    function bid(uint8 _price) public canDoActionBid {
        ...
        // ** Start code here.  ** /
        // + Check the price with currentPirce and minimumStep. Revert if invalid.
        if(currentPrice + rule.minimumStep > _price) // <Số tiền trả giá> < currentPrice + rule.minimumStep 
            revert();
    ...
    function announce() public canDoActionAnnoucement {
    ...
    function getDeposit() public onlyBidder canDoActionGetDeposit {
